...
//test!!
            ...
            if (ft.opts.width) {
                ft.handles.width = ft.group.rect(ft.width, -10, 10, 10)
                ft.handles.width.drag(function(dx, dy, x, y) {
                    var tdx = this.getTransformedDrag( ft, this.data('op').x, this.data('op').y, x, y );
                    console.log(tdx.x)
                    ft.width = Math.max(0, tdx.x + this.data('ow'));
                    ft.apply();
                    asyncCallback(['width change']);
                }, function( x, y ) {
                    this.data('op', this.getCursorPoint( x, y ));
                    this.data('ow', ft.width);
                })
            }
        ...
        ft.hideHandles = function(opts) {
            ...
            if (ft.handles.width) {
                ft.handles.width.remove();
                ft.handles.width = null;
            }
